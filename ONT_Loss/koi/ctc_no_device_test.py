...
def _simple_lattice_fwd_bwd(alpha, beta_T, beta_stay, beta_move, stay_scores, move_scores, S:semiring):
    ...
    # Call the host function directly (assumed to be compatible with CPU)
...
def logZ_cu(stay_scores, move_scores, target_lengths, S:semiring=Log):
    return LogZ.apply(stay_scores, move_scores, target_lengths, _simple_lattice_fwd_bwd, S)
...
def fwd_scores_cu_sparse(Ms, idx, v0, S:semiring=Log, K=1):
    ...
    idx = idx.to(dtype=torch.int)  # Removed device-specific operation
...
def bwd_scores_cu_sparse(Ms, idx, vT, S:semiring=Log, K=1):
    ...
    idx_T = idx.flatten().argsort().to(dtype=torch.int)  # Removed device-specific operation
...
def logZ_fwd(Ms, idx, v0, vT, S:semiring=Log, K=4):
    if Ms.dtype != torch.float32:
        raise NotImplementedError("Only fp32 supported")
    if K != 1:
        raise NotImplementedError("Only K=1 supported")
...
class _LogZ(torch.autograd.Function):
    ...
    def forward(ctx, Ms, idx, v0, vT, S:semiring, K):
        logZ, Ms_grad = logZ_fwd(Ms, idx, v0, vT, S, K)
